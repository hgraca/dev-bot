...
- **Delete → prune self-heal (live — audit-28/29 FAIL)**: deleting a memory note must stop it surfacing in search. Write a probe note (as in the round-trip above), wait until `search-memories` returns it, then `rm` it. opencode 1.18.26's watcher does **not** reliably dispatch `file.deleted` for external (bash) deletions, so let the session go idle (wait up to a minute) and re-run `search-memories` for the marker: the `session.idle` `reindex-memories-prune-idle` hook (`qmd cleanup && qmd update`, no embed) must have self-healed it and the marker must be gone. Cross-check `.agents/logs/qmd-index.log` for the idle prune hook entry and `.agents/logs/hooks.log` for `watcher-diag` lines — these record rejected delete-lookalike watcher payloads; **quote them in the report**, they reveal the real event shape and decide whether `deletedFileFromWatcher` should be adapted or the idle prune kept as the answer. FAIL if the stale entry persists after an idle cycle with no prune entry and no watcher-diag line (idle events not delivered, hook not wired, or the self-heal chain broken). If the watcher-diag shows a trivial matcher fix (e.g. event value is `remove` not `unlink`), note it as the suggested fix.
