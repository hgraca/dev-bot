...
- **Delete → prune self-heal (live — audit-28/29 FAIL, fixed audit-30)**: deleting a memory note must stop it surfacing in search. Write a probe note (as in the round-trip above), wait until `search-memories` returns it, then `rm` it. Neither harness delivers a delete event for external (bash) deletions — opencode 1.18.26 emits no watcher unlink and claudecode has no delete event — so the self-heal runs **at session start**: on the next new session, the `session.created` `reindex-memories-prune-start` hook (`qmd cleanup && qmd update`, no embed) must have pruned it and the marker must be gone. Cross-check `.agents/logs/qmd-index.log` for the prune-start entry. FAIL if the stale entry persists after a fresh session with no prune entry (session.created not dispatched or the hook not wired). A note deleted mid-session may briefly surface until the next session start — expected, not a defect.
