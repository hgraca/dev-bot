...
@test "devbot init scaffolds a project directory from the test template" {
  ...
  [ -f "$test_project/.agents/memory/active/mcp.md" ]
  ...
  [ "$(readlink "$test_project/.agents/memory/latent/global")" \
    = "$PROJECT_ROOT/src/agentic/memory/global-memories" ]
  ...
  MODULES_WITH_COMMANDS=(dev devteam explore memory signoz)
  for MODULE in "${MODULES_WITH_COMMANDS[@]}"; do
    local source_commands="$PROJECT_ROOT/src/agentic/${MODULE}/commands"
    local linked_commands="$test_project/.opencode/commands/${MODULE}"
    [ -L "$linked_commands" ] || fail "commands/${MODULE} is not a symlink"
    [ "$(readlink "$linked_commands")" = "$source_commands" ] || fail "commands/${MODULE} points wrong target"
    diff <(ls "$source_commands" | sort) <(ls "$linked_commands" | sort) \
      || fail "commands mismatch between source and symlink"
  done

  # ── Verify skills match source ─────────────────────────────────────────
  local source_skills_dir="$PROJECT_ROOT/src/agentic"
  local linked_skills="$test_project/.opencode/skills/devbot"
  ...
  if [[ -d "$PROJECT_ROOT/vendor/addyosmani/agent-skills" ]]; then
    # agents symlink
    local addy_agents="$test_project/.opencode/agents/addyosmani"
    ...
    # skills symlink
    local addy_skills="$test_project/.opencode/skills/addyosmani"
  ...
  # Specific checks for key plugin-tool pairs
  grep -qF "agent-communication" "$test_project/.opencode/plugins/on-session_idle-agent-communication.ts" \
    || fail "agent-communication plugin target mismatch"

  grep -qF "auto-recover" "$test_project/.opencode/plugins/on-session_error-auto-recover.ts" \
    || fail "auto-recover plugin target mismatch"

  grep -qF "guards" "$test_project/.opencode/plugins/on-tool_execute_before-guards.ts" \
    || fail "guards plugin target mismatch"
