...
" 2>/dev/null || true)
  ...
  local format_json_tool="${dev_bot_root}/src/agentic/format-json/tools/format-json.sh"
  if [[ -f "${format_json_tool}" ]]; then
    bash "${format_json_tool}" "${config_file}" 2>/dev/null || true
  fi

  # Ensure config exists with modules key
  ...
  done < <(python3 "${MODULE_DIR}/../../_shared/read_jsonc.py" "${config_file}" modules | \
    python3 -c "
import json, sys
