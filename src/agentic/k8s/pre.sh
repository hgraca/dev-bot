...
if ! command -v curl >/dev/null 2>&1; then
    _fatal "curl is required to download kubeconform and kube-linter binaries"
    exit 1
fi
_ok "curl found"
...
if ! command -v tar >/dev/null 2>&1; then
    _fatal "tar is required to extract release archives"
    exit 1
fi
_ok "tar found"

# Optional: network check (warn only — user may be offline, already installed)
if curl --connect-timeout 10 --head https://github.com >/dev/null 2>&1; then
