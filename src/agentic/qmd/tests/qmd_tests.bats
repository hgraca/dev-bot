...
@test "init.sh runs qmd cleanup before update (no embed)" {
  # Guard audit-26 NOTE-6: qmd status reported orphaned chunks because reinit
  # (init.sh) never ran `qmd cleanup` — only the reindex tool does. The stub qmd
  # records its argv; assert cleanup precedes the index build. BM25-only: init
  # must never call `qmd embed` (ADR 20260913072905-qmd-bm25-only-no-model-...).
  # XDG_CACHE_HOME is sandboxed so nothing touches the developer's real cache.
  local sandbox
  sandbox="$(mktemp -d)"
...
SCRIPT
  ...
  # BM25-only: init must never embed.
  refute_output --partial "embed"
