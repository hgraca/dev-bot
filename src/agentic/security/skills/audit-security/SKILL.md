...
- Performing security review of web application code
...
4. **Security Misconfiguration** — exposed debug info, verbose errors
5. **Cryptographic Failures** — weak hashing, hardcoded secrets
6. **XSS** — output escaping in templates
7. **Insecure Deserialization** — deserializing untrusted input
8. **Using Components with Known Vulnerabilities** — dependency audit
9. **Insufficient Logging** — security events not logged
10. **SSRF** — user-controlled URLs in HTTP requests

## 3. Language-Specific Checklists

The OWASP categories above apply universally; the checklists below map them to concrete, language-specific checks. Run the checklist for each language the project uses.
...
#### Quick Scan Commands (Safe, Read-Only)
...
## 4. Authentication and Authorisation Deep-Dive
...
## 6. Vulnerability Finding Format
