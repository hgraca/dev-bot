...
```
[BLOCKED] prompt_opener_missing — first sentence was: "<verbatim first sentence>". Re-delegate with Analyse/Investigate/Diagnose/Research imperative per `devbot:agent-communication` SKILL.
```
...
- Load the `devbot:search-memory` context skill. Search for relevant gotchas, ADRs, patterns, and past solutions in the problem domain.
- Load the `devbot:search-code` context skill. Locate the relevant code paths, definitions, callers, and callees.
...
2. **Map the relevant codebase** — identify all files, classes, functions, and interfaces in the blast radius. Trace callers, callees, and dependencies. Use the tools from `devbot:search-code` skill: `codebase_context` for first-look evidence, `call_graph` for relationships, `grep` for exact matches.
