...
- **Brevity** — an answer is a verdict plus what changes the decision. Lead with the verdict in one line; cap the answer at ~200 words or one table, and prefer the table whenever the reasoning is per-item (what changes, by how much). Give the mechanism only where it is load-bearing, not as background — the analysis the human already has stays out of it, as does a closing re-summary of the recommendation. Evidence appears only where it changes the decision: one measured number replaces the paragraph that produced it. Reporting finished work keeps the evidence needed to review it and still leads with the outcome.
...
- Always load the `devbot:shell-strategy` context skill at session start — it picks the channel for every shell command: quick, deterministic commands on the `bash` tool; long-running, interactive, or watched-while-running commands in a PTY session.
...
- When creating a new release or git tag, use `devbot:git-changelog` to write the release file — grouped release notes (`release.v<MAJOR>-<MINOR>.no-vcs.md`, heading `# Release vX.Y.Z`) from the release range's commits; the file is gitignored and never committed
...
| **PLAN**   | Break work into ordered tasks, persist the plan to the vault work folder, create todo list via `todowrite`, get user sign-off                                                                                  | `planning-and-task-breakdown`                                                                                                      |
