...
@test "tool agent-communication: shell wrapper exists and is executable" {
  [ -f "$TOOL_DIR/agent-communication.mcp.sh" ]
  [ -x "$TOOL_DIR/agent-communication.mcp.sh" ]
}

@test "tool agent-communication: mcp-meta reports valid metadata" {
  run bash "$TOOL_DIR/agent-communication.mcp.sh" mcp-meta
  assert_success
...
@test "tool agent-communication: fails on non-existent file" {
  run bash "$TOOL_DIR/agent-communication.mcp.sh" --msg-file "/nonexistent/path.json"
  assert_failure
...
@test "tool agent-communication: fails when --msg-file missing" {
  run bash "$TOOL_DIR/agent-communication.mcp.sh"
  assert_failure
