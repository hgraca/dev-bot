#!/usr/bin/env bash
# =============================================================================
# src/agentic/codebase-memory/pre.sh
# Prerequisites check for the codebase-memory module.
...
main() {
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    if [[ "${major}" -lt 18 ]]; then
      _fatal "Node.js >= 18 is required (found v${node_ver}) — codebase-memory-mcp requires node >= 18: https://nodejs.org/"
      exit 1
    fi
  else
  ...
  _ok "codebase-memory prerequisites check complete"
}
