...
- Architect's plan (path or inline content)
- Brief
...
### Step 1: Read the plan — MUST

Before writing the review, read the plan file. The delegation prompt provides at minimum the plan's file path (canonical `<issue-folder>/backlog.md`). The plan content may also be embedded inline in the prompt — if so, use it directly. If only the path is provided, read the file at that path before proceeding. The review must cite specific line numbers and sections from the plan (see Template: Evidence field); this is impossible without reading the file.

If the plan content is neither embedded nor reachable at the provided path, signal `[BLOCKED] plan_not_accessible — provide plan content inline or verify file path` and do not proceed.

### Step 2: Write the review

Proceed to write the review report below. All subsequent steps assume the plan has been read and its content is available for citation.
...
When recommendation involves third-party library — namespace, class name, method signature, configuration option — critic MUST cite source-of-truth for API in recommendation itself. Acceptable cites same as in `devbot:make-plan` SKILL's "Third-party library integration" section:

- library's dependency manifest autoload/module section (e.g. `composer.json`, `package.json`) with file path + line reference, OR
- specific source file in vendored dependency tree with file path + symbol name, OR
- library's official README or documentation URL with symbol name visible at that URL.

Hypothetical recommendations forbidden — if cannot verify API surface, finding should describe problem and require architect to verify, NOT propose specific replacement (e.g. "cited namespace is unverified — architect must verify against the dependency manifest before re-submission" rather than "use `Approve\Approvals` instead").

This rule symmetric with architect-side rule in `devbot:make-plan` SKILL. Together they form closed loop: architect must cite when proposing; critic must cite when recommending.
