...
Use `devbot:thinking` skill for temp files. Promote useful findings from `thinking/` to latent/ or the issue artifact before session close.
...
The former `devbot:make-plan` document sections map onto the hierarchy as follows. In the **epic path** "epic-level" means the epic `backlog.md` header; in the **story path** "story-level" means the single `backlog.md` header.
...
Each technical action (at any level) is specified with this shape — adapted from the former `devbot:make-plan` Step template. Use only the fields that apply; omit empty fields rather than padding them.
...
- When proposing to inline or remove an interface whose concrete class is declared immutable or final (e.g. PHP's `readonly class`), the backlog MUST include an explicit test-double strategy. Many mocking libraries cannot subclass final/immutable classes (engine fatal at runtime). Acceptable strategies: (a) keep the interface and mock it; (b) relax the immutability/final modifier on the concrete; (c) provide a hand-written fake under `tests/Support/`. See `latent/global/` or `latent/learnings/` for the project's specific gotcha (e.g. "Mockery cannot mock `readonly class`").
...
**Scope**: language stdlib / built-in symbols are exempt — they are part of the language, not third-party. The rule applies to any symbol whose top-level namespace or module matches a dependency declared in the project's dependency manifest (excluding the language runtime and its standard extensions). The Pre-[FINISHED] Hygiene Gate mechanically enforces this; add an inline citation, add a `[VERIFY: …]` marker, or remove the reference.
...
- The critic reviews the **combined `backlog.md`** — both the task/AC decomposition and the technical actions — and saves to `<work-folder>/PLAN-REVIEW-YYYY-MM-DD-NNN.md` per `devbot:review-plan` skill (NEVER use a `CRITIC-*.md` prefix).
...
- **Post-revision format-md — MUST**: after any architect revision to `backlog.md` (whether addressing critic findings or self-correcting), the orchestrator MUST run `devbot:format-md` on the work folder before re-delegating to the critic. Table misalignment after edits masks content errors and slows review. This step is the orchestrator's responsibility, not the architect's.
...
2. Produce a post-epic retrospective per `devbot:make-retrospective` skill. Save to `<work-folder>/retrospective/implementation.md`.
...
5. **Plan-size check** — count lines (`wc -l`). If the backlog exceeds 900 lines for a single-component scope, identify every code sketch over 30 lines. For each: either summarise as `### Behaviour` (bulleted) + `### Contract` (signatures only) + `### Implementation file` (path reference), removing the full body; OR justify verbatim inclusion with a **discriminating** one-line reason in the technical action's `**Notes**`. A discriminating justification cites a property that does NOT generalise to other verbatim blocks in the same backlog. Generic phrases ("exact assertion values required", "contract MUST be precise", "needed for clarity") do NOT discriminate and are invalid. Valid examples:
 ...
 - "byte-identical approval test fixture — any whitespace change breaks the test"
  - "regex with 4 lookarounds — escaping rules differ across language versions"
  - "exact exception class hierarchy required because the test framework's `expectException` checks `instanceof`, and the inheritance chain is contested in the backlog"
 ...
 Specific block types that do NOT pass the discriminating bar by default and MUST be summarised rather than included verbatim, unless the architect cites an additional contract-bearing property the standard binding does not capture:
  ...
  - **Directory trees** — the contract is in namespace/import declarations + the File path(s) of technical actions; the tree is convenience. Default to a textual summary (e.g. "Component groups: X, Y, Z; ports under `Core/Port/<Component>/`; see the module-resolution/autoload block at line N") plus the module-resolution block. A tree passes the bar only if the architect cites a property the standard binding does not capture (e.g. "non-standard layout deviates from the language's module-resolution conventions in module M and the deviation MUST be reviewed line-by-line").
   - **Configuration scaffolds** (dependency manifest, test-runner config, framework config) — the contract is the version-pinned dependency or named extension; surrounding structure is boilerplate. Pass the bar only if a specific non-default key requires byte-identical reproduction (cite the key by name).
...
Any line in the output is an unverified third-party reference. The check FAILS if any such line is found and not exempt (language stdlib symbols exempt). Report the match count and exempt-line count separately. The grep pattern above matches backslash-prefixed namespaces; adapt it to the language's namespace/import syntax.
...
9. **Language syntax verification** — every non-standard type syntax or language construct in code blocks or type declarations (generic closure signatures, intersection types, union types with unsupported combinations, syntax specific to un-released language versions, etc.) MUST be verified against the official language documentation or the project's dependency manifest language-version constraint. If the syntax is not valid for the project's language version, replace it with the correct equivalent (e.g. a generic closure type for a callable, or a docblock/annotation callable signature) and add an inline comment documenting the resolution. Report: every non-standard syntax found; each entry's status (`verified-valid`, `corrected`, or `no-non-standard-syntax-found`). Zero `uncertain` entries required to pass. Language built-in types and standard types from the project's language version are exempt.
10. **Third-party YAML config key verification** — for every YAML configuration snippet referencing a third-party receiver, processor, exporter, or extension (OpenTelemetry collector components, Kubernetes resource specs, Helm chart values), the architect MUST verify the exact key names, values, and required fields against the official documentation URL cited in the action's `**Notes**`. This check is required when the YAML uses identifiers under vendored-dependency namespaces, OTel component names (`kubeletstats`, `k8s_events`, `k8sattributes`, `prometheus`, `filelog`, `hostmetrics`, `file_storage`, etc.), or any key-value pair documented at a URL in the action's Notes. Report: every YAML block found; for each, the documentation URL verified against; status (`docs-matched`, `inline-citation-only`, or `unverified`). Zero `unverified` entries required to pass. YAML blocks whose keys are fully enumerated in the same technical action's `**Behaviour**` with explicit per-key documentation citations are exempt.
11. **Dockerfile native-extension build-dependency verification** — for any technical action that installs a native extension from source in a Dockerfile (e.g. `pecl install` for PHP, `pip install` of a source package, `gem install` with native extensions), verify that the RUN block includes build tool installation BEFORE the install command, OR that the architect has verified the base image includes build tools and documented the verification with a URL or file-path citation in the action's `**Notes**`. This check catches the common pitfall where a production Dockerfile purges `-dev` packages before the extension build step and the base image lacks build tools (`autoconf`, `gcc`, `make`, `pkg-config`). Report: every such source-install reference found; for each, the build-tool strategy (`install-before-compile`, `base-image-includes` with citation, or `unverified`). Zero `unverified` entries required to pass.
...
> 11. Docker native-extension deps: K source-install references, 0 unverified
...
- Unexpected situation → use `devbot:exception-handling` skill for structured recovery.
