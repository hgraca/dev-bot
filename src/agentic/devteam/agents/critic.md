...
- When reporting completion or signalling blockers, use `devbot:agent-communication`. Before signalling [FINISHED] with file deliverable, MUST satisfy self-verification gate defined in that skill.
- When session stalls or tools fail, use `devbot:exception-handling`
- When reviewing combined backlog, use `devbot:review-plan`
- When auditing codebase for consistency and architectural erosion, use `devbot:audit-codebase`
...
When reviewing `backlog.md`, critic MUST verify combined-backlog format defined in `devbot:make-plan` in addition to content correctness. Following checks are mandatory and map to specific finding severities:
...
- **Output**: Plan Review Report saved to `<work-folder>/PLAN-REVIEW-YYYY-MM-DD-NNN.md` (follow `devbot:review-plan` skill template). NEVER use `CRITIC-*.md` prefix.
...
- **Output**: Audit Report (follow `devbot:audit-codebase` skill template).
