...
- Record decisions, lessons, and patterns — captured automatically by remember-session plugin on idle. For stakeholder answers and new rules, record immediately to `latent/PDRs/` or `latent/ADRs/` (too important to defer). After writing to either file, run `devbot:format-md` on file to align table columns.
