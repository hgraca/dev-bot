...
Guard rules match **per command segment**: the command is split on shell
separators (`;`, `&&`, `||`, `|`, `&`, newline, `(`, `)`, redirects) and each
rule regex is anchored to the START of a segment — so a dangerous command must
actually be the one being invoked. A safe command whose TEXT merely contains a
pattern (e.g. `echo "rm -rf ..."` or a filename like `rm -rf.txt`) is NOT blocked.

Two cases keep the substring match (fail closed, because the danger hides in
arguments): (1) the segment's command is a command-runner that can execute
