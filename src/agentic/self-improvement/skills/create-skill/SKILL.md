...
- Optimize instructions with `optimize-instructions`
...
After skill content finalized, run description improver (separate script) to optimize triggering. Then run `optimize-instructions` to optimize skill file for clarity, token efficiency, and pattern compliance.
...
- **Give freedom** when multiple approaches are valid. Explain _why_ — an agent that understands purpose makes better context-dependent decisions.
- **Be prescriptive** when operations are fragile, consistency matters, or a specific sequence must be followed.

**Provide defaults, not menus**: Pick a default tools/approach and mention alternatives briefly rather than presenting equal options.

**Favor procedures over declarations**: Teach the agent _how to approach_ a class of problems, not _what to produce_ for one instance. A method for any analytical query beats a specific SQL query.
...
After the skill content is finalized (iteration loop complete) and the description has been optimized, run `optimize-instructions` on the created skill's SKILL.md to ensure it meets clarity, token efficiency, and pattern compliance standards.

### Procedure

1. Load the `optimize-instructions` skill
...
- **Optimize instructions**: `optimize-instructions` is a skill loaded on demand, no subagents or CLI tools required. Run it after the skill content is finalized.
...
- Optimize instructions with `optimize-instructions` (optional, user decides)
