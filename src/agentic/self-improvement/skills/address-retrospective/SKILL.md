...
- After `devbot:make-retrospective` skill produces retro file
...
Retro file path (`$1`). File follows `devbot:make-retrospective` template. Contains at minimum **What Needs Improvement** section with actionable items.
...
3. Record change as decision in `latent/PDRs/` or `latent/ADRs/` following `devbot:memory-management` skill routing (section 2).
