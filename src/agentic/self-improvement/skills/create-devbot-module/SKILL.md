...
- TypeScript plugins under `hooks/opencode/`.
- During install, `install.sh` symlinks the TS file into `.opencode/plugins/devbot/` so OpenCode discovers it.
- The symlink uses a relative path so it survives repo moves.

### Claude Code hooks

- Bash scripts under `hooks/claudecode/`.
- Must be registered manually in `~/.claude/settings.json` — each script's header documents the config.

### Git hooks

- Git hook targets under `hooks/git/`.
- Wired into the project's `.git/hooks/` directory.
...
1. **"What event does the hook react to?"** — OpenCode events: `session.idle`, `session.error`, `file.edited`, `tool.execute.before`, `tool.execute.after`, etc. Claude Code events: `PostToolUse`, `PreToolUse`, `Stop`, `Notification`. Git events: `post-commit`, `post-checkout`, `pre-push`.

2. **"What does the hook do?"** — Describe the hook's behavior.

3. **"Does it have another hook?"** — If yes, repeat questions 1-2.

**Implementation** for each hook:

- **OpenCode hook**: Create `hooks/opencode/<hook-name>.ts`. TypeScript plugin using `@opencode-ai/plugin` SDK. Export a default function that handles the event. Include comment header with event name and behavior description.

- **Claude Code hook**: Create `hooks/claudecode/<hook-name>.sh`. Bash script with header documenting the config snippet for `~/.claude/settings.json`.
...
1. **"What is the tool name?"** — Lowercase, hyphens. E.g., `search-code`, `format-md`.
...
If yes, use the `create-skill` skill to create each skill. Keep repeating until the user has no more skills to create.

**Implementation**: Skills go under `skills/<skill-name>/SKILL.md`. Each skill follows the canonical SKILL.md format with YAML frontmatter (name, description required). Use `create-skill` skill for each one.
...
| 8   | Hooks: OpenCode hooks are TS plugins, Claude Code hooks are bash                     |           |
