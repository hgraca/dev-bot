...
After all tasks in backlog completed (`implement-story` skill step 3), orchestrator produces retrospective:
...
2. **Immediately** load and execute `address-retrospective` skill with the retro file path. This step is NOT optional and MUST NOT be deferred. Retrospective is incomplete until `address-retrospective` has run.
3. Report completion to stakeholder only after both steps above are done.

**Gate**: If step 2 is skipped, the retrospective is [PARTIAL] — not [FINISHED]. Orchestrator MUST NOT signal [FINISHED] until `address-retrospective` has been invoked.
...
- `address-retrospective` MUST be invoked as step 2 of the Post-Iteration Retrospective procedure — not after, not deferred, not optional. Skipping it makes the retrospective [PARTIAL].
- `address-retrospective` MUST process all three retro sections: **What Needs Improvement**, **Process Observations**, and **Follow-Up Tasks**. Explicitly acknowledge each section — even if no actionable items exist, state "No actionable items" rather than silently skipping.
