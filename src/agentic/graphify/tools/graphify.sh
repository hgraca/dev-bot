...
query_graph_json() {
  ...
  if [[ ! -f "$graph_file" ]]; then
    echo "Error: graph.json not found. Run 'graphify.sh update' first." >&2
    exit 1
  fi
  ...
  case "$cmd" in
    ...
    query)
      if [[ $# -eq 0 ]] && is_pipe; then
        ...
        question="$(cat | tr -d '\n')"
        if [[ -z "$question" ]]; then
          echo "Error: query requires a question as argument or piped input." >&2
          exit 1
        fi
    ...
    path)
      if [[ $# -lt 2 ]]; then
        echo "Error: path requires two arguments: source and target." >&2
        exit 1
      fi
      exec graphify path "$1" "$2" "$@"
      ;;

    explain)
      if [[ $# -lt 1 ]]; then
        echo "Error: explain requires a node name." >&2
        exit 1
      fi
