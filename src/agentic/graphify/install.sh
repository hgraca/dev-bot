  ...
  if ! command -v uv >/dev/null 2>&1; then
    if command -v curl >/dev/null 2>&1; then
      ...
      curl -LsSf https://astral.sh/uv/install.sh | sh 2>/dev/null || {
        _fatal "uv installation failed. Install manually: curl -LsSf https://astral.sh/uv/install.sh | sh"
        exit 1
      }
      ...
      brew install uv 2>/dev/null || {
        _fatal "uv installation failed."
        exit 1
      }
    else
      _fatal "uv is required but not installed."
