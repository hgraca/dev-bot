...
# Tests for the tree.sh bash entrypoint.
...
# ── No --md or --json options present ──────────────────────────────────────────

@test "no --md flag: passed as dir and treated as non-existent, not as flag" {
  run bash "$TOOL" "$FIXTURES" "--md"

  assert_success
  assert_output --partial "> skipped non-existent paths: --md"
  assert_output --partial "file_a.txt"
}
