...
# Waits for the shared Svelte MCP gateway (the docker compose service declared in
...
main() {
  ...
  while ! curl -sf -o /dev/null --max-time 2 \
    ...
    if [[ ${retries} -ge 30 ]]; then
      _skip "svelte gateway not reachable at ${SVELTE_MCP_URL} after 30s — MCP server will be unavailable"
      return 0
    fi
    sleep 1
  done
