...
Scans a PHP project using message-bus conventions to trace call chains:
...
Requirements:
  ...
  - PHP project with message-bus library conventions
"""
...
class GraphScanner:
    ...
    def __init__(
        ...
        self._dispatch_patterns = self._load_config(
            ...
            or "../../conf/php/message-bus-dispatch-patterns.php",
            default=[],
        )
        ...
        self._all_php_files = []
        # True when PHP CLI was unavailable, so the map is declaration-based
        # only and may be incomplete (audit-27 FAIL-1: empty map reported as
        # success). Surfaced as a "degraded" marker in the output JSON.
        self._php_degraded = False
    ...
    def _load_config(self, path, default=None):
        ...
        If absolute, uses it as-is. A REQUIRED config (no default) that is
        missing is a broken install — fail loudly instead of silently producing
        an empty map. Optional configs (default provided) degrade gracefully.
        """
        ...
        if not os.path.isfile(config_path):
            if default is not None:
                return default
            print(
                f"  ERROR: required config not found: {config_path}\n"
                "  The use-case-map tool ships its configs under conf/php/ — a\n"
                "  missing file means a broken or partial install. Re-run devbot\n"
                "  init/reinit, or pass --arch-config pointing at the file.",
                file=sys.stderr,
            )
            sys.exit(1)
        try:
    ...
    def _discover_code_units(self):
        ...
        if not php_cmd:
            self._php_degraded = True
            print(
                "  WARNING: PHP CLI not found — message-bus discovery degraded.\n"
                "  Install PHP >=8.0 locally or Docker (matching composer.json's\n"
                "  PHP version) for full command/handler/event resolution; without\n"
                "  it, only declaration-based discovery runs (conf/php/message-bus-types.php).",
                file=sys.stderr,
            )
        ...
        if "docker" in php_cmd[0]:
            reflect_script = "/devbot-conf/php/message-bus-types.php"
            php_root = "/app"
        else:
        ...
        try:
            ...
            with open(tmp_list) as fh:
                try:
                    ...
                    # Docker PHP failed to launch (no daemon, image missing) —
                    # the map will be declaration-based only; mark degraded.
                    self._php_degraded = True
        ...
        if proc.returncode != 0:
            # Reflection failed (docker daemon absent, PHP error) — the map is
            # declaration-based only; mark degraded (audit-27 FAIL-1).
            self._php_degraded = True
            print(
    ...
    def _find_php_command(self):
        ...
        if os.path.isfile(composer_path):
            ...
            print(
                ...
                "  following the pattern in conf/php/message-bus-types.php",
                file=sys.stderr,
            )
            return None
        ...
        # The message-bus reflection script ships with the tool under
        # conf/php/ — mount it into the container so the dockerized PHP can
        # run it (the project root itself has no conf/php/).
        self._conf_php_dir = os.path.normpath(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "../../conf/php"
            )
        )
        try:
            docker_cmd = (
                [
                    ...
                    f"{self.root}:/app",
                    "-v",
                    f"{self._conf_php_dir}:/devbot-conf/php:ro",
                ]
                + extra_mount
                + [
                    "-w",
...
class UseCaseMapBuilder:
    ...
    def build(self, title="", subtitle=""):
        ...
        result = {"$schema": "", "title": title, "subtitle": subtitle, "items": sections}
        # audit-27 FAIL-1: an empty map produced without a PHP CLI must not be
        # presented as success. Mark the output as degraded so the caller (MCP
        # layer) can distinguish "nothing found" from "couldn't look properly".
        if self.s._php_degraded and not sections:
            result["degraded"] = True
            result["degraded_reason"] = (
                "PHP CLI not found — only declaration-based discovery ran; "
                "install PHP >=8.0 (or Docker) for full command/handler/event resolution"
            )
        return result
...
def main():
    ...
    parser.add_argument(
        "--dispatch-patterns",
        default="",
        help="Dispatch pattern config file (PHP array of {pattern, mode}). Default: conf/php/message-bus-dispatch-patterns.php",
    )
    ...
    result = builder.build(
        title=args.title
        or f"Architecture: {args.component + ' ' if args.component else ''}Overview",
