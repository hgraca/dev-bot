...
Generate an architecture diagram (UseCaseMap) from a PHP codebase that follows the get-e/message-bus conventions and explicit-architecture layering.
...
- get-e/message-bus library conventions (Command, CommandHandler, Event, EventListener, Query, QueryHandler interfaces)
- Explicit architecture directory structure (see `explicit-architecture` skill)
...
src/agentic/docs/tools/UseCaseMap/use-case-map.sh [flags]

# Or if wired in PATH:
use-case-map.sh [flags]
```
...
1. Generate the JSON: `use-case-map.sh -o docs/use-case-map.json`
2. Copy the visualizer: `use-case-map.sh --copy-visualizer docs/use-case-map-app.html`
...
1. **Load config**: reads `conf/php/structure-explicit-architecture.php` for directory layout, `conf/php/get-e-message-bus-dispatch-patterns.php` for dispatch methods, `conf/php/http-clients-types.php` for HTTP client base classes
2. **Index PHP files**: walks all configured directories (src, entry points, ports, adapters)
3. **PHP reflection**: runs `conf/php/get-e-message-bus-types.php` via PHP CLI (local or dockerized) to discover all Command→Handler, Query→QueryHandler, and Event→Listener mappings with full inheritance chain resolution
...
- `explicit-architecture` — directory structure and layer conventions this tool depends on
- `app-map` — interactive app map for browsing the same architecture visually
- `graphify` — knowledge graph tool used for structural discovery
