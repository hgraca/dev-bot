#!/usr/bin/env python3
"""Insert entries into the 'modules' section of a JSONC file while preserving all comments.

Usage:
  merge_modules_jsonc.py <jsonc_file> <entries_file>
    Reads entries from entries_file (a JSON object of module declarations)
    and merges each into the "modules" key of jsonc_file.
...
Exit codes:
  ...
  0 - NO_MODULES: file has no modules section (created one)
  1 - ERROR:      other failure

This is the modules-section counterpart of merge_mcp_jsonc.py.
"""
...
def main():
    ...
    bounds = _find_field_value_end(text, "modules")
    if bounds is None:
        # No "modules" section — create one before the last closing brace.
        ...
        modules_entry = '\n' + indent + '"modules": ' + modules_content + '\n' + closing_indent
