...
if [[ -f "${MCP_CONFIG}" ]]; then
  # Use remove_mcp_key.py to surgically remove dev-bot MCP servers. Only STALE
  # servers are dropped: removing one that already matches its module template
  # makes init re-append it at the end of the map, reordering keys and breaking
...
" 2>/dev/null | while IFS= read -r mcp_key; do
        if [[ -n "${mcp_key}" ]]; then
          if [[ -f "${REMOVE_MCP_PY}" && -f "${IS_CURRENT_PY}" ]] \
            && python3 "${IS_CURRENT_PY}" "${MCP_CONFIG}" "${MCP_FILE}" "${mcp_key}" 2>/dev/null; then
