...
| --------------- | ----------------- | ------------------------------------------------------------- |
| aws             | `aws-mcp`         | AWS API access via a local proxy (disabled by default)        |
| chrome-devtools | `chrome-devtools` | Browser inspection, console, network, performance, Lighthouse |
| codebase-index  | `codebase-index`  | Semantic code search, implementation lookup, call graph       |
| context7        | `context7`        | Version-accurate library/framework documentation              |
| graphify        | `graphify`        | Codebase knowledge graph querying                             |
...
| qmd             | `qmd`             | Markdown knowledge-base search (semantic + keyword)           |
...
The `tools-mcp` module's `devbot-tools` MCP server is the only MCP whose tools come from DevBot itself — it exposes the DevBot tool scripts as MCP tools, each self-describing via its `mcp-meta` subcommand. The 11 tools are generated via `devbot list tools -a`:
