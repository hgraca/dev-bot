...
| memory              | reindex-memories    | Rebuild the memory index in the background with the configured engine (qmd: cleanup && update [&& embed]; mdctx: build of project + global indexes). Coalesces concurrent runs via a pidfile. Pass the argument 'status' to check whether a reindex is running without launching one. |
