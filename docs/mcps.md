...
| memory              | search-memories     | Search the memory vault and return full file bodies. Fast keyword (BM25) search across memories indexed by the configured engine (qmd or mdctx) — no GPU or LLM models required. Branch-aware: refreshes the index first when it was built for a different git branch or checkout.    |
