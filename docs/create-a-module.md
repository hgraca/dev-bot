...
Skills are `SKILL.md` files under `skills/<skill-name>/`, each with YAML frontmatter (`name` and `description` are required). Load the `create-skill` skill when authoring one.
...
- **OpenCode** — TypeScript plugins under `hooks/opencode/`, using the `@opencode-ai/plugin` SDK. Named `on-<event>-<module>.ts`.
- **Claude Code** — bash scripts under `hooks/claudecode/`, with a header documenting the `~/.claude/settings.json` snippet.
