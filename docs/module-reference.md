...
| datasources         | 1   | —   | —   | —   | ✓   | Shared MCP Toolbox gateway — agents query a project's databases (MySQL/MariaDB, Postgres, SQLite, MongoDB, Redis) with one free-form tool per datasource; declared once in the global config, opted into per project, and only reachable databases are registered                        |
