...
**Layer 1: per-project gating (the default, config-only).** A project pays only for the servers it uses. Module enablement stays the canonical gate (ADR `20260908213000` module-gate policy unchanged); projects additionally opt out of _heavy per-instance servers_ (chrome-devtools, playwright, graphify, signoz…) through a per-project selection surfaced at init. opencode's runtime `mcp.<name>.enabled: false` is honored (verified: config is load-once, not hot-reloaded) and remains the local escape hatch for disabling an inherited server without unregistering it.
...
**Follow-up work.** (1) Standardize versions pinning across enabled binaries (done for chrome-devtools/playwright in T1.2; extend to svelte/next-devtools when enabled). (2) T2.3 — per-project server selection at init. (3) T3.2 — implement gating in the manifest/registration layer. (4) T3.3 — compose file + http wiring per shared server, rollout signoz first; verify transport support per server before moving it. (5) T4.1 — a `devbot` runtime-footprint report to keep these numbers measurable.
