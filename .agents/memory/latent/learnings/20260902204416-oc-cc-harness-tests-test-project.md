...
Operational notes: containers use **pid-suffixed names** (`devbot-test-cc-$$` / `devbot-test-oc-$$`) with EXIT/INT/TERM trap force-removal, and each run mounts an **isolated per-run copy** of the fixture at `/app` (shared helpers in `test-lib.sh`, sourced by both launchers). cc and oc — or two runs of the same harness — can therefore run in **parallel**: they no longer race over `.devbot.project.jsonc`, the harness wiring dirs, the nested `.git`, or the audit-report NN sequence (the old shared mount made every parallel run corrupt the others' state — both containers ended up launching claudecode). The host composer cache is mounted in (cross-platform: composer's own `cache-dir`, else Linux `~/.cache/composer` / macOS `~/Library/Caches/composer`). On exit the launcher syncs the run's audit report (next free NN on the real tree) and devbot+staged harness logs back to the real fixture under `.agents/logs/<report-id>/`; inner scripts stage claude/opencode harness logs into `/app/.agents/logs/harness` first because they live only inside the container.
