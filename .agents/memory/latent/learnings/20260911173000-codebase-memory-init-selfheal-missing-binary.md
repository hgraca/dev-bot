...
The module no longer installs a host binary at all: the server runs in the shared gateway container and the session-start hook calls it over MCP (`tools/mcp-index.py`), so `init.sh`, `install.sh`, `update.sh`, `pre.sh` and the Node ≥ 18 host prerequisite were all removed, and `mcp.json` carries only the http URL. There is no host CLI left to self-heal, so this specific failure mode is gone. The general rule above still holds for a module whose engine really is a globally-installed host CLI — it simply no longer describes this one. See ADR `20260914222815`'s 2026-09-22 amendment.
