...
Delegate to @scout with explicit keyword list and `devbot:gather-context` skill
```
...
**Step 2: Load stage-appropriate context skill**

| Stage  | Context skills loaded                                                                   |
| ------ | --------------------------------------------------------------------------------------- |
| DEFINE | `spec-driven-development` + `doubt-driven-development` + `idea-refine` + `interview-me` |
| PLAN   | `planning-and-task-breakdown`                                                           |
| BUILD  | `incremental-implementation` + `test-driven-development` + `devbot:software-development`       |
...
- Load the `devbot:software-development` context skill
...
4. Acquires lock, injects hidden synthetic prompt:
    ...
    Invoke the `devbot:remember-session` skill, execute its steps exactly ONCE...
    ```
5. DevBot runs `devbot:remember-session` skill silently (no narration):
...
- DevBot loads the `devbot:remember-session` context skill
...
- Same `devbot:remember-session` post-commit plugin fires on idle
