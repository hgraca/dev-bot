...
## 3. Communication Protocol (`agent-communication`)
...
| **PLAN**   | Break work into ordered tasks, create todo list, user sign-off | `planning-and-task-breakdown`, `make-plan`               |
| **BUILD**  | Execute tasks one at a time; load tech skills                  | `incremental-implementation`, `test-driven-development`  |
...
## 5. Planning Workflow (`make-plan` → `make-plan` SKILL)
...
## 6. Implementation Workflow (`implement-story` SKILL)
...
- Run retrospective (`make-retrospective`)
...
- **Session start**: DevBot runs `gather-context` skill. TeamLead delegates to `@scout` with explicit keywords.
...
- `remember-session` captures session state on wrap-up / idle.
- `search-memory` and `search-memories` tools for recalling past learnings.
